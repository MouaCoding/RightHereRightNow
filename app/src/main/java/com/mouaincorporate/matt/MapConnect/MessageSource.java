...
public class MessageSource {
    ...
    public static void saveMessage(Messages message, String conversationId) {
        ...
        msg.put("ReceiverPhone", message.getPhone());
        if(message.getSender().equals(FirebaseAuth.getInstance().getCurrentUser().getUid())) {
            msg.put("ReceiverName", message.receiverName);
            msg.put("SenderName", message.senderName);
        }
        else {
            msg.put("ReceiverName", message.senderName);
            msg.put("SenderName", message.receiverName);
        }

        try{msg.put("ReceiverProfile",message.receiverProfile);}catch (Exception e){}
